...
public:	
	...
	PerspectiveCamera(Point3 o, double d, Vec3 h, Vec3 v, Vec3 w, double hh, double hw, double ap) : 
		origin(o), distance(d), horizontal(h), vertical(v), w(w), halfWidth(hw), halfHeight(hh), aperture(ap) {}

	Ray getRay(double u, double v) {
		v = -halfWidth + 2*halfWidth*v;
		u = -halfHeight + 2*halfHeight*u;
		return Ray(origin, horizontal*u + vertical*v - w*distance, aperture);
	}
